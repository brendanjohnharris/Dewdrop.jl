...
by `(seed, step, neuron)`: identical across runs, threads and devices. The seed is domain-separated,
so sharing one with a [`PoissonDrive`](@ref) still leaves the two streams independent.
"""
struct WhiteNoise{T}
    σ::T
    seed::UInt64
end
WhiteNoise(σ::Real; seed = 0) = WhiteNoise(float(σ), domain_seed(seed, DOMAIN_NOISE))
export WhiteNoise
