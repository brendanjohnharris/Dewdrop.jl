...
@testset "CUDA GPU backend (M6)" begin
    ...
    if !CUDA.functional()
        ...
        function ei_net(arch; T = 200.0)
            ce = fixed_prob(arch, 200, 200, 0.1; weight = 0.5, delay = 15, seed = UInt64(1), sources = 1:160, allow_self = false)
            ci = fixed_prob(arch, 200, 200, 0.1; weight = -1.0, delay = 15, seed = UInt64(2), sources = 161:200, allow_self = false)
            return DewdropNetwork(m, 200; input = 0.0, tspan = (0.0, T), arch = arch,
        ...
        @testset "COBA conductance synapses + randomized init on GPU" begin
            function coba(arch)
                ...
                ce = fixed_prob(arch, 500, 500, 0.05; weight = 0.6, delay = 1, seed = UInt64(1), sources = 1:400)
                ci = fixed_prob(arch, 500, 500, 0.05; weight = 6.7, delay = 1, seed = UInt64(2), sources = 401:500)
                return DewdropNetwork(mc, 500; input = 0.0, tspan = (0.0, 200.0), arch = arch,
        ...
        @testset "deterministic subthreshold LIF: GPU ≈ CPU exact propagator" begin
            ...
            sg = solve(DewdropNetwork(m, 64; input = 15.0, tspan = (0.0, 100.0), arch = Dewdrop.GPU()),
                FixedStep(0.1); record = (V = Trace(:V),))
            sc = solve(DewdropNetwork(m, 64; input = 15.0, tspan = (0.0, 100.0), arch = Dewdrop.CPU()),
                FixedStep(0.1); record = (V = Trace(:V),))
        ...
        @testset "Int32 indices + Float32 path on the GPU (scatter perf knobs)" begin
            function ei(::Type{T}, N, IT; Tend = 150.0) where {T}
                ...
                ce = fixed_prob(Dewdrop.GPU(), N, N, 0.1; weight = T(0.5), delay = 15, seed = UInt64(1), sources = 1:(4N ÷ 5), allow_self = false, index_type = IT)
                ci = fixed_prob(Dewdrop.GPU(), N, N, 0.1; weight = T(-1.0), delay = 15, seed = UInt64(2), sources = (4N ÷ 5 + 1):N, allow_self = false, index_type = IT)
                DewdropNetwork(mm, N; input = T(0), tspan = (T(0), T(Tend)), arch = Dewdrop.GPU(),
                    projections = (Projection(DeltaSynapse(), ce), Projection(DeltaSynapse(), ci)),
        ...
        @testset "compacted scatter on the GPU: bit-identical to edge-parallel + faster when sparse" begin
            function ein(N, p; Tend = 100.0, w = 0.01)
                ...
                ce = fixed_prob(Dewdrop.GPU(), N, N, p; weight = w, delay = 15, seed = UInt64(1), sources = 1:(4N ÷ 5), allow_self = false)
                ci = fixed_prob(Dewdrop.GPU(), N, N, p; weight = -w, delay = 15, seed = UInt64(2), sources = (4N ÷ 5 + 1):N, allow_self = false)
                DewdropNetwork(mm, N; input = 0.0, tspan = (0.0, Tend), arch = Dewdrop.GPU(),
                    projections = (Projection(DeltaSynapse(), ce), Projection(DeltaSynapse(), ci)),
        ...
        @testset "fused megakernel makes the GPU faster than the CPU at scale (M6 Tier-1)" begin
            ...
            N, T = 20_000, 100.0
            netp(arch) = DewdropNetwork(m, N; input = 0.0, tspan = (0.0, T), arch = arch,
                projections = (Projection(DeltaSynapse(),
                    fixed_prob(arch, N, N, 0.02; weight = 0.5, delay = 15, seed = UInt64(1), sources = 1:(4N ÷ 5), allow_self = false)),
                    Projection(DeltaSynapse(),
                        fixed_prob(arch, N, N, 0.02; weight = -1.0, delay = 15, seed = UInt64(2), sources = (4N ÷ 5 + 1):N, allow_self = false))),
                drive = PoissonDrive(rate = 20.0, weight = 0.5, seed = UInt64(3)))
            rec = (spikes = Spikes(),)
        ...
        @testset "ensemble batching on the GPU: bit-exact oracle + speedup vs sequential" begin
            ...
            N, B, T = 500, 8, 100.0
            netp(input, arch) = DewdropNetwork(m, N; input = input, tspan = (0.0, T), arch = arch,
                projections = (Projection(DeltaSynapse(),
                    fixed_prob(arch, N, N, 0.1; weight = 0.5, delay = 15, seed = UInt64(1), sources = 1:(4N ÷ 5), allow_self = false)),
                    Projection(DeltaSynapse(),
                        fixed_prob(arch, N, N, 0.1; weight = -1.0, delay = 15, seed = UInt64(2), sources = (4N ÷ 5 + 1):N, allow_self = false))),
                drive = PoissonDrive(rate = 20.0, weight = 0.5, seed = UInt64(3)))
            ...
            big = DewdropNetwork(m, Nb; input = 0.0, tspan = (0.0, Tb), arch = Dewdrop.GPU(),
                projections = (Projection(DeltaSynapse(),
                    fixed_prob(Dewdrop.GPU(), Nb, Nb, 0.05; weight = 0.5, delay = 15, seed = UInt64(1), sources = 1:(4Nb ÷ 5), allow_self = false)),
                    Projection(DeltaSynapse(),
                        fixed_prob(Dewdrop.GPU(), Nb, Nb, 0.05; weight = -1.0, delay = 15, seed = UInt64(2), sources = (4Nb ÷ 5 + 1):Nb, allow_self = false))),
                drive = PoissonDrive(rate = 20.0, weight = 0.5, seed = UInt64(3)))
            solve(big, FixedStep(0.1); batch = Bb, streams = 0:(Bb - 1))     # warm
            for k in 1:Bb; solve(big, FixedStep(0.1)); end                   # warm
            tbatch = minimum(@elapsed(CUDA.@sync solve(big, FixedStep(0.1); batch = Bb, streams = 0:(Bb - 1))) for _ in 1:2)
            tseq = minimum(@elapsed(CUDA.@sync (for k in 1:Bb; solve(big, FixedStep(0.1)); end)) for _ in 1:2)
