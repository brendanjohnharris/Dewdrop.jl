...
@testset "FNS: hooks + metadata" begin
    m = FNSNeuron(; C = 0.25, gL = 0.0167, VL = -70.0, VK = -85.0, Vθ = -50.0, Vr = -60.0,
...
@testset "FNS: subthreshold fixed point (gK decays to 0 → conductance-LIF)" begin
    ...
    @test only(sol.state.state.V) ≈ m.VL + I / m.gL rtol = 1e-3
    @test only(sol.state.state.w) ≈ 0.0 atol = 1e-9       # gK relaxes to 0
end
