...
@testset "Turbo backend (LoopVectorization extension)" begin
    ...
    function check(model, name; T = 300.0, dt = 0.1f0, N = 2000)
        conn = fixed_prob(Dewdrop.CPU(), N, N, 0.05f0; weight = 0.05f0, delay = steps(2), seed = UInt64(1))
        prob = DewdropNetwork(
            model, N; input = 400.0f0, tspan = (0.0f0, Float32(T)),
    ...
    check(
        AdEx(;
            C = 200.0f0, gL = 10.0f0, EL = -70.0f0, VT = -50.0f0, ΔT = 2.0f0, Vr = -58.0f0,
    ...
    @testset "Turbo with COBA conductance synapses" begin
        ...
        conn = fixed_prob(Dewdrop.CPU(), 1000, 1000, 0.05f0; weight = 0.6f0, delay = steps(1), seed = UInt64(2))
        prob = DewdropNetwork(
            m, 1000; input = 0.0f0, tspan = (0.0f0, 150.0f0),
    ...
    @testset "with-extension error paths" begin
        ...
        @test sum(
            solve(
                DewdropNetwork(m, 64; input = 400.0f0, tspan = (0.0f0, 30.0f0)),
                FixedStep(0.1f0); step = :turbo, v0 = (-70.0f0, -50.0f0)
            ).spike_count
        ) ≥ 0
    end
end
